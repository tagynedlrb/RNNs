...
from GRUModel_orbd_delta import TraceDataset, GRUModel
...
def train_model():
    ...
    model = GRUModel(hidden_dim=HIDDEN_DIM).to(device)
    ...
    base_dir = os.path.join("trained_gru_orbd_delta", today)
    ...
    with open(loss_log_path, "w", newline="") as f_loss:
        ...
        for epoch in range(EPOCHS):
            ...
            for x, y in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
                ...
                loss = (
                    criterion(out["sign"].transpose(1, 2), y_sign) +
    ...
    with torch.no_grad(), open(test_log_path, "w", newline="") as f_test:
        ...
        for x, y in test_loader:
            ...
            # 배치별 손실 계산
            batch_loss_sign = nn.functional.cross_entropy(out["sign"].transpose(1, 2), y_sign, reduction='none').mean(dim=1)
            ...
            total_batch_loss = batch_loss_sign + batch_loss_o3 + batch_loss_o2 + batch_loss_o1 + batch_loss_o0
