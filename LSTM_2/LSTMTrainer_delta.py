...
from LSTMModel_delta import TraceDataset, LSTMModel
...
def train_model():
    ...
    print("Delta vocab size:", dataset.vocab_size)
    ...
    base_dir = os.path.join("trained_lstm_lba_delta", today)
    ...
    with open(loss_log_path, "w", newline="") as f_loss:
        ...
        for epoch in range(EPOCHS):
            ...
            for x, y in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
                ...
                logits = out["delta"]  # [B, T, V]
                logits = logits.transpose(1, 2)  # [B, V, T] -> CE loss expects [B, V, T]
    ...
    torch.save({
        "model_state_dict": model.state_dict(),
        "vocab_size": dataset.vocab_size,
        "index_to_delta": dataset.idx_to_delta,
    ...
    with torch.no_grad(), open(test_log_path, "w", newline="") as f_test:
        ...
        for x, y in test_loader:
            ...
            logits = out["delta"].transpose(1, 2)  # [B, V, T]

            loss = criterion(logits, y)
            total_test_loss += loss.item()

            pred = torch.argmax(out["delta"], dim=-1)

            for i in range(x.size(0)):
                writer_test.writerow(["input"] + x[i].squeeze(-1).tolist())
                writer_test.writerow(["target"] + y[i].tolist())
                writer_test.writerow(["output"] + pred[i].tolist())
